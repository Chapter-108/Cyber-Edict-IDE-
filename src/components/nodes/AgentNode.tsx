...
const designMarkerStyle: CSSProperties = {
  position: 'absolute',
  top: 6,
  right: 6,
  width: 6,
...
export const AgentNode = memo(({ data, selected }: NodeProps<AgentFlowNode>) => {
  ...
  const cardStyle = useMemo<CSSProperties>(
    ...
    [isProvince, selected, hovered, accentColor],
  );
  ...
  const topBarStyle = useMemo<CSSProperties>(
    ...
    [accentColor, data.status],
  );

  const bodyStyle = useMemo<CSSProperties>(
    () => ({
      padding: isProvince ? '12px 14px' : '10px 12px',
      position: 'relative',
    }),
    [isProvince],
  );
  ...
  return (
    ...
    >
      ...
      <div style={bodyStyle}>
        ...
        <div style={modelTextStyle}>{data.soulConfig.model}</div>
        ...
        {mode === 'design' ? <div style={designMarkerStyle} /> : null}
      </div>
